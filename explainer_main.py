...
import evolvegcn.egcn_h_exp as egcn_h
import models
...
def fix_cg_dict(cg_dict):
    ndarray
    cg_dict["feat"] = cg_dict["feat"]
    return cg_dict
...
def main():
    ...
    if prog_args.writer:
        ...
        if os.path.isdir(path) and prog_args.clean_log:
            ...
            if not input("Are you sure you want to remove this directory? (y/n): ").lower().strip()[
                   :1] == "y": sys.exit(1)
            shutil.rmtree(path)
    ...
    ckpt = io_utils.load_dynamic_checkpoint(prog_args)
    cg_dict = ckpt["cg"]
    fix_cg_dict(cg_dict)  # todo Arash: implement this
    input_dim = cg_dict["feat"][0].shape[1]  # was ndarray [d*n*n] | Tensor [n*n]
    num_classes = cg_dict["pred"].shape[1]  # was ndarray [d*n*n_class] | Tensor [n*f_dim]
    ...
    if graph_mode:
        ...
        #     "cls_l2_dropout": 0
        # }
        model = egcn_h.EGCN(prog_args, activation=torch.nn.RReLU(), device='cuda')
    ...
    model.load_state_dict(ckpt["gcn_dict"])

    # Arash: Explainer is here. This is the model which explains.
    # Arash: You need to add T to it.
    # Create explainer
    ...
    if prog_args.explain_node is not None:
        ...
        if prog_args.multinode_class >= 0:
            ...
            # Arash: default is here. They are explaining nodes 400 to 700 with 5 increments.
            # Arash: so this is a single-instance classification for a set of nodes.
