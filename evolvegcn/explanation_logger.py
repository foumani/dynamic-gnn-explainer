...
import evolvegcn.utils as utils
...
class Logger:
    ...
    def log_minibatch(self, predictions, true_classes, loss, plot_results=False, label_binarizer=None, **kwargs):
        ...
        if self.minibatch_done % self.minibatch_log_interval == 0:
            ...
            logging.info(
                self.set + ' batch %d / %d - partial error %0.4f - partial loss %0.4f - partial MRR  %0.4f - partial MAP %0.4f' % (
            ...
            logging.info(self.set + ' batch %d / %d -  partial tp %s,fn %s,fp %s' % (
            ...
            support_sum = 0
            for cl in range(self.num_classes):
                cl_support, cl_precision, cl_recall, cl_f1 = self.calc_eval_measures_per_class(tp, fn, fp, cl)
                macro_precision += (cl_precision)
                macro_recall += (cl_recall)
                macro_f1 += (cl_f1)
                support_sum += cl_support
                logging.info(
                    self.set + ' batch %d / %d - measures partial for class %d - precision %0.4f - recall %0.4f - f1 %0.4f ' % (
                    self.minibatch_done, self.num_minibatches, cl, cl_precision, cl_recall, cl_f1))

            macro_precision = macro_precision / self.num_classes
            macro_recall = macro_recall / self.num_classes
            macro_f1 = macro_f1 / self.num_classes
            logging.info(
                self.set + ' batch %d / %d - measures partial macro avg - precision %0.4f - recall %0.4f - f1 %0.4f - support %d' % (
                    self.minibatch_done, self.num_minibatches, macro_precision, macro_recall, macro_f1,
                    support_sum))
            logging.info(self.set + ' batch %d / %d - Batch time %d ' % (
                self.minibatch_done, self.num_minibatches, (time.monotonic() - self.lasttime)))

        self.lasttime = time.monotonic()

    def log_epoch_done(self, set_name):
        eval_measure = 0
        ...
        support_sum = 0
        for cl in range(self.num_classes):
            cl_support, cl_precision, cl_recall, cl_f1 = self.calc_eval_measures_per_class(self.conf_mat_tp,
                                                                                           self.conf_mat_fn,
                                                                                           self.conf_mat_fp, cl)
            macro_precision += (cl_precision)
            macro_recall += (cl_recall)
            macro_f1 += (cl_f1)
            support_sum += cl_support
            logging.info(self.set + ' measures for class %d - precision %0.4f - recall %0.4f - f1 %0.4f ' % (
        ...
        macro_recall = macro_recall / self.num_classes
        macro_f1 = macro_f1 / self.num_classes
        logging.info(self.set + ' measures macro avg - precision %0.4f - recall %0.4f - f1 %0.4f ' % (
            macro_precision, macro_recall, macro_f1))
        logging.info(self.set + ' Total epoch time: ' + str(((time.monotonic() - self.ep_time))))

        if str(self.args.target_class) == 'macro_AVG':
            if self.args.target_measure == 'Precision' or self.args.target_measure == 'prec':
        ...
        return eval_measure, self.losses.mean()
    ...
    def get_MAP(self, predictions, true_classes, do_softmax=False):
        probs = predictions.argmax(dim=1)
        probs = probs.detach().cpu().numpy()
        true_classes_np = true_classes.long().detach().cpu().numpy()

        return precision_score(true_classes_np, probs, average='micro')
        # if do_softmax:
        #     probs = torch.softmax(predictions,dim=1)[:,1]
        # else:
        #     probs = predictions
        ...
        # print(true_classes_np)
        # print(true_classes_np.shape)
        # print(predictions_np)
        # print(predictions_np.shape)
        # return average_precision_score(true_classes_np, predictions_np, pos_label=1)
