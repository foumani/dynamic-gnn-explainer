...
class Sp_GCN(torch.nn.Module):
    ...
    def forward(self, A_list, Nodes_list, nodes_mask_list, raw_data_list=None):
...
class Sp_GCN_LSTM_A(Sp_GCN):
    ...
    def forward(self, A_list, Nodes_list=None, nodes_mask_list=None, raw_data_list=None):
        last_l_seq = []
...
class Classifier(torch.nn.Module):
    def __init__(self, args, out_features=2, in_features=None):
        ...
        self.mlp = torch.nn.Sequential(torch.nn.Linear(in_features=num_feats,
                                       ...
                                       activation,
                                       torch.nn.Dropout(p=args.gcn_parameters['cls_l1_dropout']),
                                       torch.nn.Linear(in_features=args.gcn_parameters['cls_feats'],
                                                       out_features=args.gcn_parameters['cls_l2_feats']),
                                       activation,
                                       torch.nn.Dropout(p=args.gcn_parameters['cls_l2_dropout']),
                                       torch.nn.Linear(in_features=args.gcn_parameters['cls_l2_feats'],
