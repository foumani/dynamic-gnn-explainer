import evolvegcn.egcn_utils as u
import torch
...
class EGCN(torch.nn.Module):
    def __init__(self, args, activation, device='cpu', skipfeats=False):
        ...
        # in_features = args.layer_2_feats
        self.classifier = Classifier(args, 3, feats[2]).to(self.device)
        self._parameters.extend(list(self.classifier.parameters()))

    def parameters(self):
        return self._parameters

    def forward(self, adj_list, nodes_list, nodes_mask_list, node_indices=None):
...
class Classifier(torch.nn.Module):
    def __init__(self, args, out_features=2, in_features=None):
        ...
        if in_features is not None:
            ...
            num_feats = args["layer_2_feats"] * 2
        ...
        self.mlp = torch.nn.Sequential(torch.nn.Linear(in_features=num_feats,
                                       ...
                                       activation,
                                       torch.nn.Dropout(p=args["cls_l1_dropout"]),
                                       torch.nn.Linear(in_features=args["cls_feats"],
                                                       out_features=args["cls_l2_feats"]),
                                       activation,
                                       torch.nn.Dropout(p=args["cls_l2_dropout"]),
                                       torch.nn.Linear(in_features=args["cls_l2_feats"],
