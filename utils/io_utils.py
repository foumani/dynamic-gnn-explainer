...
# todo arash: remove unnecessary code
...
# todo arash: complete this
def load_dynamic_checkpoint(args):
    """
    Loads a dynamic graph checkpoint
    :param args:
    :return: checkpoint: dict
    """
    print("loading model")
    filename = os.path.join(args.ckptdir, "checkpoint.pth.tar")
    if os.path.isfile(filename):
        print("=> loading checkpoint '{}'".format(filename))
        ckpt = torch.load(filename)
    else:
